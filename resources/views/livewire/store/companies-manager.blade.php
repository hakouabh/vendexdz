<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 font-sans">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">@lang('Link Companies')</h1>
            <p class="text-slate-500 text-sm mt-2 font-medium">@lang('Supercharge your Vendex store with official integrations.')</p>
        </div>
        
        <div class="relative w-full md:w-96">
            <input type="text" wire:model.live="search" class="block w-full pl-4 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-bold placeholder-slate-400 focus:ring-2 focus:ring-indigo-500 shadow-sm transition" placeholder="@lang('Search delivery company...')">
        </div>
    </div>

    @if($installed->count() > 0)
        <div class="mb-12">
            <h2 class="text-lg font-bold text-slate-900 mb-5 flex items-center gap-2"><i class="ri-check-double-line text-green-500"></i> @lang('Installed & Active')</h2>
    ...
    <div>
        <h2 class="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2"><i class="ri-apps-2-line text-indigo-500"></i> @lang('Explore Companies')</h2>
            ...
            @foreach($available as $app)
                <div class="bg-white rounded-[24px] p-1 border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition duration-300 flex flex-col h-full">
                    <div class="p-5 flex-1">
                        ...
                        <p class="text-xs text-slate-500">@lang('Connect') {{ $app->name }} @lang('to sync data.')</p>
                    </div>
                    <div class="p-2 mt-auto">
                        <button wire:click="openInstallModal('{{ $app->app_id }}', '{{ $app->name }}')" 
                            ...
                            <i class="ri-download-cloud-2-line text-lg"></i> @lang('Install')
                        </button>
                    </div>
                </div>
            @endforeach
    ...
    <x-dialog-modal wire:model.live="confirmingInstallation">
        <x-slot name="title">
            @lang('Configure') {{ $this->selectedAppName }}
        </x-slot>

        <x-slot name="content">
            <div class="mb-4 text-sm text-slate-600">
                @lang('Please enter your API credentials to connect') <strong>{{ $this->selectedAppName }}</strong> @lang('to Vendex.')
            </div>
