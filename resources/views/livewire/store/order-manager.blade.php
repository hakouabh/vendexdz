  <div>
      <div class=" flex justify-between mb-8 items-center">
          ...
          <div>
               <button wire:click="$toggle('createNeworeder')" class="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-slate-200">
                  ...
                  <span class="text-[11px] font-bold text-slate-600">{{ $createNeworeder ? 'Cancel Creation' : 'Order Creator' }}</span>
              </button>
          </div>
      </div>
      @if($createNeworeder)
      ...
      <div class="">
          ...
          <div wire:loading.remove>
              ...
              @livewire('store.order.inconfermation-manager')

              @elseif ($currentTab === 'postponed')
              @livewire('store.order.reported-manager')

              @elseif ($currentTab === 'waiting')
              @livewire('store.order.pending-manager')

              @elseif ($currentTab === 'indelivery')
              @livewire('store.order.indelivery-manager')
              @endif
          </div>
