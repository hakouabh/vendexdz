<div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 font-sans antialiased text-left" x-data="{ open: @entangle('showForm') }">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-2 font-medium">@lang('Manage your products, variants, and stock levels across your store.')</p>
        </div>

        <div class="flex flex-col sm:flex-row gap-4">
            <div class="relative w-full md:w-80">
                <input type="text" wire:model.live="search" class="block w-full pl-4 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-bold placeholder-slate-400 focus:ring-2 focus:ring-indigo-500 shadow-sm transition" placeholder="@lang('Search products SKU or name...')">
            </div>
            <button wire:click="create" class="flex items-center justify-center gap-2 bg-indigo-600 px-6 py-3 rounded-xl text-sm font-bold text-white hover:bg-indigo-700 transition-all shadow-md shadow-indigo-100 active:scale-95">
                <i class="ri-add-line text-lg"></i> @lang('Register Product')
            </button>
        </div>
    </div>
        ...
        <div wire:key="product-{{ $p->id }}" class="bg-white rounded-[24px] p-1 border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition duration-300 flex flex-col h-full relative overflow-hidden group">
            <div class="p-6 flex-1">
                <div class="flex items-center gap-4 mb-5">
                    ...
                    <div>
                        ...
                        <span class="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">{{ $p->nickname ?? __('No Nickname') }}</span>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    <div>
                        <div class="flex items-center justify-between mb-1.5 text-[10px] font-black uppercase tracking-wide">
                            <span class="text-slate-400">@lang('Inventory Level')</span>
                            <span class="text-slate-900">{{ $p->variants->sum('quantity') }} @lang('Units')</span>
                        </div>
                    ...
                    <div class="flex items-center justify-between pt-2">
                        ...
                        <div class="flex items-center gap-1.5 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
                            ...
                            <span class="text-xs font-bold text-slate-700">{{ $p->variants->count() }} @lang('Variants')</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="p-3 bg-slate-50/50 mt-auto border-t border-slate-50 flex gap-2">
                <button wire:click="edit({{ $p->id }})" class="flex-1 py-2.5 bg-white border border-slate-200 hover:border-indigo-600 hover:text-indigo-600 text-slate-600 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2">
                    <i class="ri-edit-box-line text-lg"></i> @lang('Edit Product')
                </button>
                ...
                <button wire:click="delete({{ $p }})" class="w-12 h-10 bg-white border border-slate-200 hover:bg-red-500 hover:text-white text-slate-400 rounded-xl transition-all flex items-center justify-center">
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
        <div class="relative w-full max-w-5xl bg-white rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div class="p-8 border-b border-slate-50 flex justify-between items-center">
                <div>
                    <h3 class="text-2xl font-extrabold text-slate-900 tracking-tight">{{ $isEditMode ? __('Modify Product') : __('Register New Product') }}</h3>
                    <p class="text-slate-500 text-xs font-medium mt-1">@lang('Fill in the details to update your inventory database.')</p>
                </div>
            ...
            <form wire:submit.prevent="save" class="overflow-y-auto p-8">
                ...
                <div class="bg-indigo-50/30 rounded-[24px] p-6 border border-indigo-100/50">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-6 gap-3 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm items-end relative">
                            ...
                            <div class="space-y-1 col-span-1">
                                ...
                                <input type="text" wire:model="variants.{{$index}}.sku"
                                    class="w-full rounded-lg border-none bg-slate-50 p-2.5 text-xs font-bold outline-none">
                            ...
                            <div class="space-y-1 col-span-1">
                                ...
                                <input type="text" wire:model="variants.{{$index}}.var_1"
                                    class="w-full rounded-lg border-none bg-slate-50 p-2.5 text-xs font-bold outline-none">
                            </div>
                            ...
                            <div class="space-y-1 col-span-1">
                                ...
                                <input type="text" wire:model="variants.{{$index}}.var_2"
                                    class="w-full rounded-lg border-none bg-slate-50 p-2.5 text-xs font-bold outline-none">
                            </div>
                            ...
                            <div class="space-y-1 col-span-1">
                                ...
                                <input type="text" wire:model="variants.{{$index}}.var_3"
                                    class="w-full rounded-lg border-none bg-slate-50 p-2.5 text-xs font-bold outline-none">
                            </div>
                            ...
                            <div class="space-y-1 col-span-1">
                                ...
                                <input type="number" wire:model="variants.{{$index}}.discount"
                                    class="w-full rounded-lg border-none bg-rose-50 p-2.5 text-xs font-black text-rose-600 outline-none">
                            </div>
                            ...
                            <div class="space-y-1 col-span-1">
                                ...
                                <input type="number" wire:model="variants.{{$index}}.quantity"
                                    class="w-full rounded-lg border-none bg-emerald-50 p-2.5 text-xs font-black text-emerald-600 outline-none">
                            </div>

                            <!-- Delete button -->
                            <div class="col-span-2 md:col-span-6 mt-2 md:mt-4 flex justify-center">
                                @if(isset($v['id']))
                                <button type="button" wire:click="destroyVariant({{ $v['id'] }}, {{ $index }})"
                                    class="w-full md:max-w-[100px] h-9 rounded-lg bg-slate-50 text-slate-300 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center">
