<div class="w-full space-y-4">
            ...
            <x-dropdown align="center" width="48">
                <x-slot name="trigger">
                    <button type="button"
                        ...
                        @if($productfilter)
                        {{ $products->firstWhere('sku', $productfilter)->name ?? __('Filter Product') }}
                        @else
                        {{ __('Filter Product') }}
                        @endif
            ...
            <x-dropdown align="left" width="48">
                <x-slot name="trigger">
                    <button type="button"
                        ...
                        @if($statufilter)
                        {{ $firstStepStatus->firstWhere('fsid', $statufilter)->name ?? __('Filter Statu') }}
                        @else
                        {{ __('Filter Statu') }}
                        @endif
    ...
    <div class="space-y-3">
        ...
        <div
            ...
            open: false,
            buttonRect: null,
            goUp: false, // متغير جديد لتحديد الاتجاه
            updatePosition() {
                this.buttonRect = $el.getBoundingClientRect();
                // إذا كانت المسافة تحت الزر أقل من 250 بكسل، افتح للأعلى
                this.goUp = (window.innerHeight - this.buttonRect.bottom) < 250;
            }
        }" class="relative inline-block text-left">

                        <button @click="updatePosition(); open = !open" type="button" class="focus:outline-none">
                            ...
                            <div class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold ring-1 ring-inset cursor-pointer hover:opacity-80 transition"
                                style="background-color: {{ $order->Inconfirmation->firstStepStatu->color }}15; color: {{ $order->Inconfirmation->firstStepStatu->color }}; ring-color: {{ $order->Inconfirmation->firstStepStatu->color }}30;">
                            ...
                            @else
                            <span
                                class="text-gray-400 text-xs cursor-pointer border border-dashed border-gray-300 rounded-full px-2 py-1">Select
                                Status</span>
                            @endif
                        </button>
                        ...
                        <template x-teleport="body">
                            <div x-show="open" x-cloak x-transition @click.away="open = false"
                                ...
                                :style="goUp 
                    ? `bottom: ${window.innerHeight - buttonRect.top + 6}px; left: ${buttonRect.left}px;` 
                    : `top: ${buttonRect.bottom + 6}px; left: ${buttonRect.left}px;`">
                                <div class="py-1">
                                    <p
                ...
                <div class="flex  items-center justify-between gap-4 border-b border-gray-200 bg-white px-6 py-3">
                    <div class="grid grid-cols-1 gap-4 w-full sm:grid-cols-3">
                        ...
                        <div class="sm:col-span-1">
                            ...
                            <div class="flex rounded-lg bg-gray-100 p-1">
                                <button wire:click="$set('order_type', '1')"
                    ...
                    <div class="flex flex-col h-full max-h-[300px]">
                        <div
                            ...
                            <span class="text-[9px] bg-indigo-50 px-2 py-0.5 rounded-full text-indigo-600 font-bold">
                                {{ $activeOrder->logs->count()}} Actions
                            </span>
                        </div>
