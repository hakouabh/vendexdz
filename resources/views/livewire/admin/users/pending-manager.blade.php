<div>
    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
            <div>
                <h2 class="text-lg font-bold text-slate-900">@lang('Pendings')</h2>
                <p class="text-sm text-slate-500 mt-1">@lang('Manage your Pendings.')</p>
            </div>
            ...
            <div class="flex gap-3">
                <div class="relative w-96 group hidden md:block">
                    ...
                    <input type="text" placeholder="@lang('Search users...')" wire:model.live="search"
                    ...
                    <tr class="hover:bg-gray-50/80 transition duration-150 group">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-medium text-slate-900">{{$Pending->name}}</div>
                                    <div class="text-xs text-slate-400">{{$Pending->email}}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 font-semibold text-slate-900">{{$Pending->phone}}</td>
                        <td class="px-6 py-4 text-slate-500">{{ $Pending->created_at->format('d M Y') }}</td>
                        <td class="px-6 py-4 text-slate-500">{{ $Pending->updated_at->format('d M Y') }}</td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <button wire:click="openEditModal({{ $Pending->id }})" wire:loading.attr="disabled"
    ...
    <x-dialog-modal wire:model="isEditModalOpen" maxWidth="lg">
            ...
            <div class="px-2 pt-2 flex items-start justify-between">
                <div class="flex gap-4">
                    <div
                        ...
                        <i class="ri-hourglass-line text-2xl text-indigo-600"></i>
                    </div>
                    <div>
                        <h2 class="text-xl font-bold text-slate-900 tracking-tight">{{ __('Update Pending') }}</h2>
                        <p class="text-sm text-slate-500 mt-1">@lang('Manage Pending details, contact info, and visibility.')</p>
                    </div>
                </div>
        ...
        <x-slot name="content">
            <div class="space-y-6 px-2">
                <div>
                    <label class="text-[11px] uppercase tracking-wider font-bold text-slate-500 mb-1.5 block">@lang('Pending Information')</label>
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                            <i
                                class="ri-hourglass-line text-slate-400 group-focus-within:text-indigo-500 transition-colors"></i>
                        </div>
                        <input type="text" wire:model="name"
                            ...
                            placeholder="e.g. Vendex Mega Pending">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative group">
                            ...
                            <input type="email" wire:model="email"
                                ...
                                placeholder="manager@Pending.com">
                        </div>
                ...
                <div>
                    <label class="text-[11px] uppercase tracking-wider font-bold text-slate-500 mb-1.5 block">@lang('Pending Status')</label>
                    <label
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="text-sm font-bold {{ $is_active ? 'text-indigo-700' : 'text-slate-700' }}">
                                    {{ $is_active ? __('Active Pending') : __('Inactive Pending') }}
                                </span>
                                @if($is_active)
                                <span
                            ...
                            <p class="text-xs {{ $is_active ? 'text-indigo-600/80' : 'text-slate-500' }}">
                                {{ $is_active ? __('This Pending is currently visible to all users and accepting orders.') : __('This Pending is hidden from the public and cannot accept orders.') }}
                            </p>
                        </div>
        ...
        <x-slot name="footer">
            <div class="border-t border-slate-100 pt-4 w-full flex justify-between items-center">
                ...
                <button type="button"
                    wire:click="deletePending({{ $editingPendingId }})" wire:loading.attr="disabled"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <x-button wire:click="updatePending" wire:loading.attr="disabled"
