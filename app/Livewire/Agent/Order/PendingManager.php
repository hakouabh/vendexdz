...
class PendingManager extends Component
{
    ...
    $orders->withQueryString();
        ...
        return view('livewire.agent.order.pending-manager', ['orders' => $orders,'wilayas'=>$willayas, 'AcceptStepStatus'=>$AcceptStepStatus ,'products'=>$products,'stores'=>$stores]);
    }
    ...
    public function loadOrderData($id)
    {
        ...
        if ($this->activeOrder) {
            ...
            $this->app_id = $this->activeOrder->app_id ?? '';
