...
class OrderManager extends Component
{
    ...
    #[Url(as: 'currentTab', history: true, keep: true)]
   ...
   protected $rules = [
    ...
    {
        $this->store_id= Auth::user()->AgentStores()->first()->id; 
...
{           
    ...
    try {
        ...
        $log= \App\Models\order_logs::create([
            'oid'       => $order->oid,
            'aid'       => auth()->id() ?? 1,
            'statu_old' => 1,
            'statu_new' => 1,
            'text'      => 'Created At: ' . now()->addHours(2)->format('Y-m-d\TH:i'),
        ]);

        $order->Type()->updateOrCreate(
    ...
    {
        $this->currentTab = $tab;
        $this->dispatch('reset-all-pagination');
    }
    ...
    public function render()
    {
        ...
        return view('livewire.agent.order-manager', [
            'willayas' => $willayas,
